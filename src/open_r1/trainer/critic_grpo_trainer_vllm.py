...
CRITIC_TEMPLATE = """You are tasked with critique the provided solution to a problem and providing constructive feedback.
The critique process and judgement are enclosed within <think> </think> and <judge> </judge> tags, respectively, i.e., <think> reasoning process here </think><answer> answer here </answer>
Problem:
{}
...
REFINE_TEMPLATE = """You are tasked with refine the provided solution to a problem based on a critic feedback.
Give refined solution.
Problem:
{}
...
class Qwen2VLCriticGRPOTrainer(Trainer):
    """
    ...
    def __init__(
            ...
            model: Union[str, PreTrainedModel],
            reward_funcs: Union[RewardFunc, list[RewardFunc]],
            critic_reward_funcs: Union[RewardFunc, list[RewardFunc]],
            args: GRPOConfig = None,
            train_dataset: Optional[Union[Dataset, IterableDataset]] = None,
            eval_dataset: Optional[Union[Dataset, IterableDataset, dict[str, Union[Dataset, IterableDataset]]]] = None,
            processing_class: Optional[PreTrainedTokenizerBase] = None,
            reward_processing_classes: Optional[Union[PreTrainedTokenizerBase, list[PreTrainedTokenizerBase]]] = None,
            critic_reward_processing_classes: Optional[Union[PreTrainedTokenizerBase, list[PreTrainedTokenizerBase]]] = None,
        ...
        # Critic Reward functions
        if not isinstance(critic_reward_funcs, list):
            critic_reward_funcs = [critic_reward_funcs]
        for i, critic_reward_func in enumerate(critic_reward_funcs):
            if isinstance(critic_reward_func, str):
                critic_reward_funcs[i] = AutoModelForSequenceClassification.from_pretrained(
                    critic_reward_func, num_labels=1, **model_init_kwargs
                )
        self.critic_reward_funcs = critic_reward_funcs
        ...
        # Critic Reward processing class
        if critic_reward_processing_classes is None:
            critic_reward_processing_classes = [None] * len(critic_reward_funcs)
        elif not isinstance(critic_reward_processing_classes, list):
            critic_reward_processing_classes = [critic_reward_processing_classes]
        else:
            if len(critic_reward_processing_classes) != len(critic_reward_funcs):
                raise ValueError("The number of critic reward processing classes must match the number of critic reward functions.")

        for i, (critic_reward_processing_class, critic_reward_func) in enumerate(zip(critic_reward_processing_classes, critic_reward_funcs)):
            if isinstance(critic_reward_func, PreTrainedModel):
                if critic_reward_processing_class is None:
                    critic_reward_processing_class = AutoTokenizer.from_pretrained(critic_reward_func.config._name_or_path)
                if critic_reward_processing_class.pad_token_id is None:
                    critic_reward_processing_class.pad_token = critic_reward_processing_class.eos_token
                # The critic reward model computes the reward for the latest non-padded token in the input sequence.
                # So it's important to set the pad token ID to the padding token ID of the processing class.
                critic_reward_func.config.pad_token_id = critic_reward_processing_class.pad_token_id
                critic_reward_processing_classes[i] = critic_reward_processing_class
        self.critic_reward_processing_classes = critic_reward_processing_classes
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.accelerator.is_main_process:
            ...
            completion_ids = [out.token_ids for completions in outputs for out in completions.outputs] # 7 * 8 for 7 processes and 8 generate num
        else:
        ...
        for j, messages in enumerate(prompts):
            ...
            image_data, _ = process_vision_info(messages)
            for i in range(batch_size):
                critic_text = CRITIC_TEMPLATE.format(initial_user_prompt, completions[i])
        ...
        if self.accelerator.is_main_process:
            ...
            critic_completion_ids = [out.token_ids for completions in outputs for out in completions.outputs] # 7 * 8 for 7 processes and 8 generate num
        else:
            critic_completion_ids = [None] * len(all_critic_inputs_vllm)

        # Broadcast the completions from the main process to all processes, ensuring each process receives its
        # corresponding slice.
        critic_completion_ids = broadcast_object_list(critic_completion_ids, from_process=0)
        process_slice = slice(
            self.accelerator.process_index * len(prompts) * batch_size,
            (self.accelerator.process_index + 1) * len(prompts) * batch_size,
        )
        critic_completion_ids = critic_completion_ids[process_slice]
        # Pad the completions, and concatenate them with the prompts
        critic_completion_ids_unpad = [torch.tensor(ids, device=device) for ids in critic_completion_ids]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
                zip(self.critic_reward_funcs, self.critic_reward_processing_classes)
        ):
            reward_kwargs = {key: [] for key in inputs[0].keys() if key not in ["prompt", "completion"]}
            for key in reward_kwargs:
                for example in inputs:
                    # Repeat each value in the column for `num_generations` times
                    reward_kwargs[key].extend([example[key]] * self.num_generations)
            reward_kwargs['process'] = self.accelerator.process_index
